...
function PaymentsPage() {
  ...
  const paymentCols: Column<(typeof payments)[number]>[] = [
  ...
  const settlementCols: Column<(typeof settlements)[number]>[] = [
    ...
    { key: "status", header: "Status", render: (r) => <StatusBadge status={r.status} /> },
    {
      ...
      render: (r) => r.status === "pending" ? (
        <Button size="sm" onClick={() => toast.success(`Payout ${r.id} released`)}>Release payout</Button>
      ) : <span className="text-xs text-muted-foreground">Released</span>,
    },
  ];

  const refundCols: Column<(typeof refunds)[number]>[] = [
