...
import { bookings, dashboardStats, revenueOverview, bookingTrends } from "@/lib/mock-data";
...
function DashboardPage() {
  return (
    <>
      <PageHeader
        title="Dashboard"
        description="Snapshot of your marketplace performance."
        actions={
          <>
      ...
      <div className="space-y-6 p-6">
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
          <StatCard
            ...
            spark={bookingTrends.map((b) => b.bookings)}
          />
          <StatCard
            label="Total Revenue"
            value={`AED ${(dashboardStats.totalRevenue / 1000).toFixed(0)}K`}
            icon={Wallet}
            trend="+8.2% vs last month"
            spark={revenueOverview.map((r) => r.revenue)}
            accent="emerald"
          />
          <StatCard
            label="Active Caterers"
            value={dashboardStats.activeCaterers}
            icon={ChefHat}
            trend="+6 this week"
            spark={[140, 148, 152, 160, 168, 172, 178, 184]}
            accent="amber"
          />
          <StatCard
            ...
            icon={Users}
            trend="+128 this week"
            spark={[4200, 4380, 4520, 4680, 4820, 4980, 5120, 5231]}
          />
        </div>
        ...
        <div className="grid gap-4 md:grid-cols-3">
          <QuickTile
            title="Pending Caterer Approvals"
            value={dashboardStats.pendingCatererApprovals}
            icon={ClipboardCheck}
            to="/admin/caterers"
            hint="Awaiting document review"
          />
          <QuickTile
            title="Pending AI Menu Reviews"
            value={dashboardStats.pendingMenuReviews}
            icon={Sparkles}
            to="/admin/menu-review"
            hint="Ready for validation"
            glow
          />
          <QuickTile
            title="Open Support Tickets"
            value={dashboardStats.pendingSupportTickets}
            icon={LifeBuoy}
            to="/admin/support"
            hint="Response SLA 4h"
          />
        </div>
                ...
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={revenueOverview} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
                    ...
                    <Tooltip
                      ...
                      labelStyle={{ color: "oklch(0.85 0.02 260)" }}
              ...
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-white/5">
                  {bookings.slice(0, 6).map((b) => (
                    <tr key={b.id} className="group transition-colors hover:bg-primary/[0.04]">
...
function QuickTile({
  ...
  return (
    <Link to={to} className="group">
      <Card
        className={`relative overflow-hidden border-white/5 bg-card/60 backdrop-blur transition-all group-hover:border-primary/40 group-hover:shadow-[0_0_40px_-12px_var(--color-primary)] ${
