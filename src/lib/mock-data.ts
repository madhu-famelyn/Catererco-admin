...
export const caterers = Array.from({ length: 24 }).map((_, i) => ({
  id: `CT-${1000 + i}`,
  name: [
  ...
  owner: ["Ahmed Al Mansoori", "Fatima Hassan", "Rajesh Kumar", "Maria Silva"][i % 4],
  email: `caterer${i + 1}@example.ae`,
  phone: `+971 5${rand(0, 9)} ${rand(1000000, 9999999)}`,
  tradeLicense: `TL-${rand(10000, 99999)}`,
  vatNumber: `VAT-${rand(100000000, 999999999)}`,
  city: ["Dubai", "Abu Dhabi", "Sharjah", "Ajman"][i % 4],
  status: (["pending", "approved", "rejected", "active"] as Status[])[i % 4],
  rating: (3.5 + seededRandom() * 1.5).toFixed(1),
  bookings: rand(0, 240),
  revenue: rand(0, 320000),
  joinedAt: `2025-${String(rand(1, 12)).padStart(2, "0")}-${String(rand(1, 28)).padStart(2, "0")}`,
  documents: ["Trade License.pdf", "VAT Certificate.pdf", "Owner Passport.pdf", "Menu.pdf"],
}));
...
export const customers = Array.from({ length: 30 }).map((_, i) => ({
  ...
  status: (["active", "active", "active", "suspended"] as Status[])[i % 4],
...
export const bookings = Array.from({ length: 40 }).map((_, i) => ({
  ...
  status: (["pending", "completed", "cancelled", "in_progress"] as Status[])[i % 4],
...
export const payments = Array.from({ length: 35 }).map((_, i) => ({
  ...
  status: (["paid", "pending", "refunded", "failed"] as Status[])[i % 4],
...
export const settlements = Array.from({ length: 18 }).map((_, i) => ({
  ...
  status: (["paid", "pending"] as Status[])[i % 2],
}));
...
export const refunds = Array.from({ length: 12 }).map((_, i) => ({
  ...
  status: (["pending", "approved", "rejected"] as Status[])[i % 3],
  date: `2025-${String(rand(1, 12)).padStart(2, "0")}-${String(rand(1, 28)).padStart(2, "0")}`,
}));
...
export const reviews = Array.from({ length: 20 }).map((_, i) => ({
  ...
  targetType: (i % 2 === 0 ? "caterer" : "customer") as "caterer" | "customer",
  rating: rand(1, 5),
  content: [
...
export const supportTickets = Array.from({ length: 22 }).map((_, i) => ({
  ...
  type: (i % 2 ? "customer" : "caterer") as "customer" | "caterer",
  priority: (["low", "medium", "high", "urgent"] as const)[i % 4],
  status: (["open", "in_progress", "resolved"] as Status[])[i % 3],
  assignee: ["Unassigned", "Layla Admin", "Karim Support"][i % 3],
  createdAt: `2025-11-${String(rand(1, 28)).padStart(2, "0")}`,
}));

export const adminUsers = [
  { id: "AD-1", name: "Layla Admin", email: "layla@catermarket.ae", role: "Super Admin", status: "active" as Status, lastLogin: "2026-07-09 09:12" },
  { id: "AD-2", name: "Karim Support", email: "karim@catermarket.ae", role: "Support Manager", status: "active" as Status, lastLogin: "2026-07-08 18:04" },
  { id: "AD-3", name: "Nadia Ops", email: "nadia@catermarket.ae", role: "Operations", status: "active" as Status, lastLogin: "2026-07-07 12:33" },
  { id: "AD-4", name: "Yousef Finance", email: "yousef@catermarket.ae", role: "Finance", status: "suspended" as Status, lastLogin: "2026-06-20 10:00" },
];
...
export const notifications = Array.from({ length: 15 }).map((_, i) => ({
  ...
  status: (["sent", "sent", "sent"] as unknown as Status[])[0],
}));
